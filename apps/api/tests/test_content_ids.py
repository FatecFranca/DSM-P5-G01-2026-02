from app.content_ids import canonical_exercise_id, canonical_lesson_id, exercise_id_for, item_id_for, lesson_id_for
...
def test_canonical_ids_translate_legacy_formats():
    ...
    assert item_id_for("casa-1", 3) == "item-casa-1-3"
...
def test_seed_uses_canonical_ids(client):
    ...
    for track in content["tracks"]:
        for unit in track["units"]:
            if unit["kind"] == "letter":
                assert unit["id"] == lesson_id_for(unit["focus_letter"])
                assert [item["id"] for item in unit["items"]] == [exercise_id_for(unit["focus_letter"], item["type"]) for item in unit["items"]]
            else:
                assert [item["id"] for item in unit["items"]] == [item_id_for(unit["id"], index) for index in range(1, len(unit["items"]) + 1)]
...
def test_sync_push_stores_canonical_ids_for_legacy_clients(client, auth):
    ...
    assert [item["unit_id"] for item in client.get("/v1/progress", headers=headers).json()["items"]] == ["lesson-A"]
    assert [item["item_id"] for item in client.get("/v1/attempts", headers=headers).json()["items"]] == ["exercise-A-listen"]
    pulled = client.get("/v1/sync/pull?cursor=0", headers=headers).json()["events"]
    assert pulled[0]["payload"]["item_id"] == "exercise-A-listen" and pulled[1]["payload"]["unit_id"] == "lesson-A"
    assert not {"exercise_id", "lesson_id"} & (pulled[0]["payload"].keys() | pulled[1]["payload"].keys())
