...
export default function RootLayout() {
  ...
  useEffect(() => { Promise.all([initializeDatabase().then(getProgress), restore()]).then(([items]) => { hydrateProgress(items); setReady(true); }).catch((cause: unknown) => setError(cause instanceof Error ? cause.message : "Falha ao abrir dados locais")); }, [hydrateProgress, restore]);
