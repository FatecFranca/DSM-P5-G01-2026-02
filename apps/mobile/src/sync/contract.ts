...
export function toApiEvent(event: SyncEvent): ApiSyncEvent {
  ...
  if (event.type === "attempt") return { client_event_id: event.id, type: "attempt", occurred_at: occurredAt, payload: { client_attempt_id: p.clientAttemptId, exercise_id: canonicalExerciseId(String(p.exerciseId)), lesson_id: canonicalLessonId(String(p.lessonId)), exercise_type: normalizeExerciseType(p.exerciseType) ?? null, answer: p.answer, correct: p.correct, duration_ms: p.durationMs } };
  const completed = (p.completedTypes as string[] | undefined) ?? [];
  return { client_event_id: event.id, type: "progress", occurred_at: occurredAt, payload: { lesson_id: canonicalLessonId(String(p.lessonId)), status: p.status === "review" || p.status === "needs_review" ? p.status : p.completed || p.status === "mastered" ? "completed" : completed.length ? "in_progress" : "not_started", completed_exercises: completed.length, completed_types: completed, attempts: p.attempts ?? 0, correct_attempts: p.correctAttempts ?? 0, accuracy: p.accuracy ?? null, review_count: p.reviewCount ?? 0, last_practiced_at: p.lastPracticedAt ?? null, next_review_at: p.nextReviewAt ?? null } };
}
