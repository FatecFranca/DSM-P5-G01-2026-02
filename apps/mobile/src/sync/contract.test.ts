...
describe("contrato da API de sync", () => {
  it("converte tentativa camelCase para envelope snake_case com IDs canônicos", () => {
    ...
    expect(toApiEvent(event)).toEqual({ client_event_id: "evt-1", type: "attempt", occurred_at: "2026-01-01T00:00:00Z", payload: { client_attempt_id: "try-1", exercise_id: "exercise-A-complete-word", lesson_id: "lesson-A", exercise_type: "complete_word", answer: "cachorro", correct: true, duration_ms: 1200 } });
  });
  it("envia progresso com metadados de domínio", () => { const event: SyncEvent = { id: "p", type: "progress", payload: { lessonId: "letter-a", completedTypes: ["listen_choose", "recognize_letter"], score: 2, completed: false, updatedAt: "2026-01-01T00:00:00Z" } }; expect(toApiEvent(event)).toEqual({ client_event_id: "p", type: "progress", occurred_at: "2026-01-01T00:00:00Z", payload: { lesson_id: "lesson-A", status: "in_progress", completed_exercises: 2, completed_types: ["listen_choose", "recognize_letter"], attempts: 0, correct_attempts: 0, accuracy: null, review_count: 0, last_practiced_at: null, next_review_at: null } }); });
