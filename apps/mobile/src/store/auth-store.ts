...
import { login as apiLogin, logout as apiLogout, refresh as apiRefresh, register as apiRegister } from "../api/client";
...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  refresh: async () => { const saved = await sessionStorage.load(); if (!saved.refreshToken) throw new Error("Sessão indisponível"); const value = await apiRefresh(saved.refreshToken); await sessionStorage.save(value.accessToken, value.refreshToken); set({ status: "authenticated", accessToken: value.accessToken }); },
