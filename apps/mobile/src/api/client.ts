...
export async function pullEvents(cursor: string | null, accessToken: string): Promise<{ cursor: string; progress: LessonProgress[] }> {
  ...
  const ordered: LessonProgress["completedTypes"] = ["listen_choose", "recognize", "complete_word"];
  const progress = (data.events ?? []).filter((event) => event.type === "progress").map(({ payload: p, occurred_at }) => { const count = Number(p.completed_exercises ?? 0); return { lessonId: String(p.lesson_id), completedTypes: ordered.slice(0, count), score: count, completed: p.status === "completed", updatedAt: occurred_at }; });
